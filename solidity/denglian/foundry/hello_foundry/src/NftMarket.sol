...
contract NFTMarket is ITokenReceiver {
    IExtendedERC20 public tokenContract; // 定价代币
    ...
    constructor(address _tokenContract) {
        require(_tokenContract != address(0), "_tokenContract invalid");

        tokenContract = IExtendedERC20(_tokenContract);
    }
    ...
    function buyNFT(uint256 _goodsId) external {
        ...
        // 检查买家是否有足够的代币
        require(
            tokenContract.balanceOf(msg.sender) >= goods.price,
            "balance not enough"
        );
        ...
        // 转移代币(需授权)
        tokenContract.transferFrom(msg.sender, goods.seller, goods.price);
    ...
    // 使用 transferFromWithCallbackAndData 购买NFT的辅助函数
    function buyNFTWithCallback(uint256 _goodsId) external {
        ...
        // 调用 transferFromWithCallbackAndData 函数，将代币转给市场合约并附带 _goodsId 数据(需授权)
        bool success = tokenContract.transferFromWithCallbackAndData(
            msg.sender,
            address(this),
            goods.price,
            data
    ...
    // 实现 tokensReceived 接口，处理通过 transferFromWithCallback 接收到的代币
    function tokensReceived(
        address from,
        uint256 amount,
        bytes calldata data
        ...
        require(
            ...
            "msg.sender must be tokenContract"
        );
        ...
        tokenContract.transfer(goods.seller, amount);
