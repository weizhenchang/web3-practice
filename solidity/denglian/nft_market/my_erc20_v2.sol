...
contract MyERC20 is ERC20 {
    ...
    function transferFromWithCallback(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        if (isContract(_to)) {
            try ITokenReceiver(_to).tokensReceived(_from, _value, "") returns (bool){
                // 回调成功
            } catch {
                // 回调失败，但不回滚交易
            }
    ...
    // 带有回调功能的转账函数(附带数据)
    function transferFromWithCallbackAndData(address _from, address _to, uint256 _value, bytes calldata _data) external returns (bool success){
        require(balanceOf(_from) >= _value, "ERC20: transfer amount exceeds balance");
        require(_from != address(0), "ERC20: transfer from the zero address");
        require(_to != address(0), "ERC20: transfer to the zero address");
        require(allowance(_from, msg.sender) >= _value, "ERC20: insufficient allowance");
        ...
        // 如果接收方是合约，调用其 tokenReceived 方法
        if (isContract(_to)) {
            try ITokenReceiver(_to).tokensReceived(_from, _value, _data) returns (bool){
                // 回调成功
            } catch {
                // 回调失败，但不回滚交易
            }
